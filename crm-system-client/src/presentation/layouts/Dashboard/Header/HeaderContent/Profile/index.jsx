...
import Avatar from '@presentation/components/@extended/Avatar';
...
export default function Profile() {
  ...
  const handleLogout = () => {
    ...
    if (isAuthenticated) {
      // Use postLogoutRedirectUri to handle the redirect after logout
      instance.logoutRedirect({
        postLogoutRedirectUri: window.location.origin + '/login'
      });
    } else {
      // If not authenticated with MSAL, just navigate to login
      navigate('/login');
    }
  };
    ...
    <Box sx={{ flexShrink: 0, ml: 0.75 }}>
      ...
      >
        <Stack direction="row" spacing={1.25} alignItems="center" sx={{ p: 0.5 }}>
          <Avatar alt="profile user" src={profilePhoto} size="sm" />
                    ...
                    <Grid container justifyContent="space-between" alignItems="center">
                      <Grid item>
                        <Stack direction="row" spacing={1.25} alignItems="center">
                          <Avatar alt="profile user" src={profilePhoto ? profilePhoto : ''} sx={{ width: 32, height: 32 }} />
                          <Stack>
                            <Typography variant="h6">{full_name ? full_name : ''}</Typography>
                      ...
                      <Grid item>
                        <Tooltip title="Logout">
                          <IconButton size="large" sx={{ color: 'text.primary' }} onClick={(event) => handleLogout(event)}>
                            <Logout />
                          </IconButton>
                        </Tooltip>
                      </Grid>
